...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    } else {//Пользовалеть нажал кнопку "Отмена"
...
{
    QString s = QFileDialog::getSaveFileName(this, "Заголовок окна", QDir::current().path(), filter);
       if (s.length() > 0) {
...
//Не совсем так, при нажатии на кнопку отменить нужно, чтобы отменилось последнее действие.
//То есть после каждого изменения в виджете нужно сохранять состояние
//Создайте ещё Слот, который будет вызываться при каждом изменении в  текстовом редакторе
//когда выполняется отмена, например так:redo.push(undo.pop())
//и получается два стека один хранит то, что нужно отменять, другой хранит то, что нужно повторять, т.е. восстанавливать
...
void MainWindow::on_pushButton_6_clicked()
{
    ...
    if (!stack.empty()){
        str=stack.top();
        stack.pop();
       ui->textEdit->setText(stack.top());

    }
 }
/*
 По нажатию на кнопку Отмена нужно получить верхнее значение в стеке,
 изменить состояние виджета и удалить это значение из стека
*/
